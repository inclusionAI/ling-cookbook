...
```text
请直接帮我将正在使用的本地编程 Agent 配置为通过 Theta 使用 Ling，添加模型选项并安装 Ling GUI Agent Skill。请使用原生配置文件和工具完成。可以使用官方 skills CLI 或直接下载 Skill 文件。

连接参数：
- 供应商显示名：Ling (theta)。使用独立 provider ID ling_theta；已有等价配置且可以安全复用时可复用。
- API Key 环境变量：THETA_API_KEY。
- OpenAI Responses Base URL：https://antchat.alipay.com/v1。
- Anthropic Messages Base URL：https://antchat.alipay.com/api/anthropic。
- OpenAI Chat Completions 和 GUI Skill Base URL：https://antchat.alipay.com/v1。
...
配置 Skill 的 .env：设置 LING_BASE_URL=https://antchat.alipay.com/v1、LING_MODEL=aicloud:ling_3_flash_vl_aa_test，从受保护凭据写入 LING_API_KEY，不输出 .env 内容。确认 Agent 能发现 Skill，不在安装过程中启动 GUI 自动化。
